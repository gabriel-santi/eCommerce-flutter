...
void main() {
  ...
  group("setItem", () {
    test("when null user, writes item to local cart", () async {
      // setup
      ...
      // run
      await cartService.setItem(Item(productId: '123', quantity: 1));
      // verify
      verify(() => localCartRepository.setCart(expectedCart)).called(1);
      verifyNever(() => remoteCartRepository.setCart(any(), any()));
    });

    test("when non-null user, writes item to remote cart", () async {
      // setup
      final expectedCart = Cart({'123': 1});
